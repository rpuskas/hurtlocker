...
Minefield = function(data,bombs){
	...
	get_unclicked_neighbors_chance = function(home_state,number_of_neighbors,marked_neighbors){
		return isNaN(home_state) ? MineCell.UNEVAL : (home_state - marked_neighbors) / number_of_neighbors;
	}
	...
	return {
		...
		get_unmarked_bombs: function(){
			...
			_data.each(function(x,y){
				var cell = that.data.cell(x,y);//why is this not each item in grid?				
				if(cell.chance === MineCell.A_BOMB && cell.state === 'unclicked'){
		...
		evaluate_all_cells: function(){
			...
			_data.each(function(x,y){ 
				...
				if(cell.chance === MineCell.UNEVAL) { cell.chance = random;};
			});
		},
		
		to_s: function(){
			return data.to_s(function(x){return Utility.pad(Math.round(x.chance*100)/100)});
		},
...
MineCell = function(obj){	
	this.state = obj.state;
	this.chance = obj.chance;
};
MineCell.Create = function(v,c){
	return new MineCell({state:v,chance:c})
}
MineCell.NO_CHANCE = 0;
MineCell.A_BOMB = 1;
MineCell.UNEVAL = -1;
...
Grid = (function(){
	...
	Constr.prototype = {
		...
		min: function(get_min){
			var that = this;
			var min_cell = MineCell.Create('X',Number.MAX_VALUE);
			this.each(function(x,y){
